...
foreach ($_POST['id_servicio'] as $i => $id_servicio) {
    ...
    $ingreso = (isset($_POST['incluye_ingreso'][$i]) && $_POST['incluye_ingreso'][$i]=='SI') ? 'SI' : 'NO';
    ...
    VALUES
    ($id,$id_servicio,$precio,'$salida','$retorno','$modalidad','$ingreso','$moneda')
    ");
}
// =========================
...
if (isset($_POST['monto_multi']) && is_array($_POST['monto_multi'])) {
    ...
    foreach ($_POST['monto_multi'] as $i => $monto) {
       ...
       mysqli_query($conexion,"
INSERT INTO pagos
(id_operaciones,tipo,metodo_pago,moneda,monto,fecha)
VALUES
($id,'$tipo','$metodo','$moneda','$monto','$fecha')
");
...
<select name="servicio_adicional[<?= $i ?>][]" class="form-control" multiple>
...
let filas =
document.querySelectorAll("#bodyTours tr")
...
let filas =
document.querySelectorAll("#bodyPagos tr")
