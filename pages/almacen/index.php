...
$(function(){
  ...
  $('#id_item').on('change', function(){
   ...
   // ===== LÓGICA FINAL POR TIPO DE ARTÍCULO =====

// ocultar todos primero
$('#wrap_numserie, #wrap_color, #wrap_talla').addClass('d-none');
$('input[name="monto"]').closest('.col-md-4').hide();

// nombre del artículo
const nombreItem = $('#id_item option:selected').text().toLowerCase();

// 🔹 POLOS → SOLO TALLA
if (nombreItem.includes('polo')) {
  $('#wrap_talla').removeClass('d-none');
}

// 🔹 DUFFEL BAG → SOLO GARANTÍA (MONTO)
if (nombreItem.includes('duffel')) {
  $('input[name="monto"]').closest('.col-md-4').show();
}

// 🔹 BASTONES / SLEEPING
// no muestran nada adicional (cantidad + fecha)
